...
export class CartController {
  static async getCart(req: NextRequest) {
    try {
      ...
      return NextResponse.json({ success: true, data: cartSummary })
    } catch (error) {
  ...
  /**
   * POST /api/cart
   * Body: { sessionId, foodItemId, quantity?, variantId? }
   *
   * `variantId` is required for items that have variants (e.g. pizza sizes).
   */
  static async addToCart(req: NextRequest) {
    try {
      const { sessionId, foodItemId, quantity = 1, variantId } = await req.json()
      ...
      const cartItem = await CartModel.addItem({ sessionId, foodItemId, quantity, variantId })
      ...
      const status =
        ...
        : error.message === 'Food item is not available' ? 400
        : error.message === 'Please select a size/variant for this item' ? 400
        : error.message === 'Selected variant not found or inactive' ? 400
        : 500
  ...
  static async removeFromCart(
    ...
    try {
      ...
      return NextResponse.json({ success: true, message: 'Item removed from cart successfully' })
    } catch (error: any) {
  ...
  static async calculateDeliveryCharges(req: NextRequest) {
    try {
      ...
      return NextResponse.json({
        success: true,
        data: { distance, deliveryCharges, freeDelivery: distance <= 4 },
      })
    } catch (error) {
